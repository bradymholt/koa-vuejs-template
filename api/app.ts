...
async function init() {
  await dbInitializer.init(
    config.get("database") as PostgresConnectionOptions,
    config.get("database.seed_test_data") as boolean
  );
  ...
  const app: Koa = createKoaServer({
    ...
    controllers: (config.get("controllers") as string[]).map(item => {
  ...
  if (config.get("webpack_middleware.enabled")) {
    boostrapWebpackMiddleware(
      app,
      __dirname + config.get("webpack_middleware.config_path")
    );
  }
